...
#define TARIEF    "0%-0:96%.14%.0%((%d+)%)"
#define POWER     "1%-0:1%.7%.0%((%d+%.%d+)%*"
#define POWERTERUG "1%-0:2%.7%.0%((%d+%.%d+)%*"
...
Dsm::Dsm( Stream &port) {
  ...
  id = 0;
  tarief=0;
  laag = hoog = laagTerug = hoogTerug = gas = power = powerTerug = 0.0;  
}
...
void Dsm::parseMsg( ) {
 ...
 // get meter values
  ...
  if( ms.Match( GAS) == 1)
    ...
    if( ms.Match( TARIEF) == 1)
    tarief = atof( ms.GetCapture (buf, 0));
    
    if( ms.Match( POWER) == 1)
    power = atof( ms.GetCapture (buf, 0));
    
    if( ms.Match( POWERTERUG) == 1)
    powerTerug = atof( ms.GetCapture (buf, 0));
}
